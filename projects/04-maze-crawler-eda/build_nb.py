#!/usr/bin/env python3
"""Regenerate notebook.ipynb for the maze-crawler-eda project.
Run from the project folder: python build_nb.py
"""
import json
...
C25 = code(
    ...
    "                 color=WHITE if 'WHITE' in dir() else '#EAECEF',\n"
...
C26 = md(
    ...
    "## 10. Conclusion\n"
    "\n"
    "This notebook covered four layers of analysis across the Maze Crawler ecosystem:\n"
    "\n"
    "**Macro trends** -- episode volume and storage have grown steadily over 24 days;\n"
    "the gap between top and median scores suggests an increasingly bifurcated field.\n"
    "\n"
    "**Episode dynamics** -- reward trajectories reveal clear exploitation phases\n"
    "(momentum spikes) once agents lock on to resource-dense zones. Crystal and\n"
    "mining-node counts deplete monotonically, confirming a resource-scarcity endgame.\n"
    "\n"
    "**Efficiency** -- winners consistently achieve higher reward-per-action ratios,\n"
    "suggesting that action economy -- not raw activity -- is the primary success factor.\n"
    "\n"
    "**Strategic patterns** -- the action Markov chain shows strong self-loops on\n"
    "mining commands for winning agents, while losers exhibit more random transitions.\n"
    "Early-game dominance (Q1 reward delta) is a reliable predictor of the final outcome,\n"
    "opening the door for early-termination or forfeit heuristics.\n"
    "\n"
    "**Next steps** -- feature-engineering the observation dictionary at each step\n"
    "to build an offline RL policy, or training an imitation-learning model on\n"
    "top-agent replays.\n"
)
...
cells = [C00, C01, C02, C03, C04, C05, C06, C07, C08, C09,
...
# Sanity checks
loaded = json.load(open("notebook.ipynb", encoding="utf-8"))
assert loaded["nbformat_minor"] == 4, "nbformat_minor must be 4"
assert loaded["metadata"]["kernelspec"]["name"] == "python3", "kernelspec must be python3"
for i, cell in enumerate(loaded["cells"]):
    assert "id" not in cell, f"cell {i} has forbidden id field"
    ...
    assert not bad, f"cell {i} has non-ASCII at positions {bad[:3]}"

print(f"notebook.ipynb written: {len(loaded['cells'])} cells, all checks passed.")
